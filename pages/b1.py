...
with col1:
    ...
    with filter_column:
        ...
        checkbox1 = st.checkbox("Available", value=False, disabled=True)
        ...
        checkbox5 = st.checkbox("Reserved", value=True, disabled=True)
...
with col2:
    ...
    for book in active_books:
        ...
        open_modal = st.button("PREPARE", key=f'open-{book.Id}')
        if open_modal:
            modal.open()

        if modal.is_open():
            with modal.container():
                copies = session.query(Egzemplarz).filter_by(KsiazkaId=book.Id, Status='Zarezerwowany').all()
                st.write(f"Copies reserved: {len(copies)}")
                if st.button("Prepare"):
                    # reserve the first copy
                    copy = copies[0]
                    copy.Status = 'Przygotowany'
                    session.commit()
                    st.success("Book prepared")
